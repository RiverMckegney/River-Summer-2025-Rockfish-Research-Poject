---
title: "SU_25_River_Rockfish_Proj_Primary_V0.1"
...
# Ambient data:
ambient_sample_table <- Ambient_Samples %>%
  group_by(Ambient.Or.OAH, Consensus_Brood_Condition) %>%
  summarise(n_size = n_distinct(Patient.ID_edited), .groups = "drop")
print(ambient_sample_table)

 pdf("data-images/ambient_sample_table.pdf")
 grid.table(ambient_sample_table)
 dev.off()
 
 png("data-images/ambient_sample_table.png")
 grid.table(ambient_sample_table)
 dev.off()

# Fecundity data: 
fecundity_sample_table <- Fecundity_Samples %>%
  group_by(Ambient.Or.OAH, Fecundity_Or_Brood_Count, Fecundity_Class) %>%
  summarise(n_size = n_distinct(Patient.ID_edited), .groups = "drop")
print(fecundity_sample_table)

 pdf("data-images/fecundity_sample_table.pdf")
 grid.table(primary_sample_table)
 dev.off()
 
 png("data-images/fecundity_sample_table.png")
 grid.table(primary_sample_table)
 dev.off()
```
